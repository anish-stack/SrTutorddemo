...
exports.TeacherVerifyOtp = CatchAsync(async (req, res) => {
    try {
        ...
        if (ExistTeacher.SignInOtp !== otp || ExistTeacher.OtpExpiresTime < Date.now()) {
...
exports.TeacherVerifyPasswordOtp = CatchAsync(async (req, res) => {
    ...
    if (Teachers.ForgetPasswordOtp !== otp || Teachers.OtpExpiresTime < Date.now()) {
        return res.status(400).json({ message: 'Invalid or expired OTP' });
    }
...
exports.AddProfileDetailsOfVerifiedTeacher = CatchAsync(async (req, res) => {
    try {
        ...
        // console.log(userId)
        const FetchProfileExist = await TeacherProfile.findOne({ TeacherUserId: userId });
        ...
        const { FullName, DOB, Gender, AlternateContact, PermanentAddress, CurrentAddress, isAddressSame, Qualification, TeachingExperience, ExpectedFees, VehicleOwned, TeachingMode, AcademicInformation, latitude, longitude, RangeWhichWantToDoClasses } = req.body;

        // Validate that all required fields are present and not empty
        if (!FullName || !DOB || !Gender || !AlternateContact || !PermanentAddress || !CurrentAddress ||
            !Qualification || !TeachingExperience || !ExpectedFees || !TeachingMode || !AcademicInformation ||
            !latitude || !longitude || !RangeWhichWantToDoClasses) {
        ...
        const requiredAddressFields = ['HouseNo', 'LandMark', 'District', 'Pincode'];
        for (const field of requiredAddressFields) {
            if (!PermanentAddress[field] || !CurrentAddress[field]) {
                return res.status(400).json({ message: 'Address fields are missing or incomplete' });
            }
        }

        // Additional validation for ExpectedFees and RangeWhichWantToDoClasses
        if (typeof ExpectedFees !== 'number' || ExpectedFees <= 0) {
            return res.status(400).json({ message: 'Expected Fees must be a positive number' });
        }

        if (typeof RangeWhichWantToDoClasses !== 'number' || RangeWhichWantToDoClasses <= 0) {
            return res.status(400).json({ message: 'Range Which Want To Do Classes must be a positive number' });
        }

        // Validate AcademicInformation
        for (const element of AcademicInformation) {
            const classExists = await Class.findById(element.ClassId);

            if (!classExists) {
                return res.status(400).json({ message: `Class with ID ${element.ClassId} does not exist` });
            }

            // Extract subject names from classExists.Subjects
            ...
            const allSubjectsValid = element.SubjectNames.every(subject => classSubjects.includes(subject));

            if (!allSubjectsValid) {
                return res.status(400).json({ message: 'Subjects in Academic Information are invalid' });
            }
        }
...
`
        ...
        res.status(200).json({
            success: true,
            data: teacherProfile,
            message: 'Profile details Saved successfully. OTP has been sent to your email.'
        });
    } catch (error) {
...
<div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 8px; border: 1px solid #ddd; padding: 20px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    ...
    <main style="padding: 20px;">
        ...
        <p style="font-size: 16px; line-height: 1.5; color: #333;">
            You are a highly qualified teacher with a <strong>${Teachers.TeachingExperience}</strong> teaching experience and an expected fee of ₹${Teachers.ExpectedFees}. You will be providing <strong>${Teachers.TeachingMode}</strong> and are available for classes within a <strong>${Teachers.RangeWhichWantToDoClasses} km</strong> radius. 
        </p>
        <p style="font-size: 16px; line-height: 1.5; color: #333;">
            Your academic qualifications include:
            <ul>
                ${Teachers.AcademicInformation.map((info, index) => `<li><strong>${classNames[index]}</strong>: ${info.SubjectNames.join(', ')}</li>`).join('')}
            </ul>
        </p>
        <p style="font-size: 16px; line-height: 1.5; color: #333;">
...
exports.GetTeacherProfileId = CatchAsync(async (req, res) => {
    try {
        ...
        const teacherProfile = await TeacherProfile.findOne({ TeacherUserId: TeacherId });
        if (!teacherProfile) {
        ...
        await redisClient.set(`TeacherProfile:${TeacherId}`, JSON.stringify(teacherProfile), 'EX', 3600); // Cache for 1 hour
...
exports.GetAllTeacherProfile = CatchAsync(async (req, res) => {
    try {
        ...
        // Cache the profile data
        await redisClient.set(`TeacherProfile`, JSON.stringify(teacherProfile), 'EX', 3600); // Cache for 1 hour
    ...
    try {
        ...
        if (query.TeachingExperience && results.length > 0) {
            ...
            results = results.filter(teacher => {
                const dbExperience = parseInt(teacher.TeachingExperience.match(/\d+/)[0], 10); // Extract the numeric part
        ...
        if (query.VehicleOwned && results.length > 0) {
            ...
            results = results.filter(teacher => teacher.VehicleOwned === vehicleOwnedQuery);
        ...
        if (query.Subject && query.Subject.length > 0 && results.length > 0) {
            ...
            results = results.filter(teacher => {
                const subjects = teacher.AcademicInformation.map((item) => item.SubjectNames);
        ...
        if (query.MinRange && query.MaxRange && results.length > 0) {
            ...
            results = results.filter(teacher =>
                teacher.ExpectedFees >= feeQuery.$gte && teacher.ExpectedFees <= feeQuery.$lte
            );
        ...
        // 8. Filter by isBestFaculty
        // if (typeof query.isBestFaculty !== 'undefined' && results.length > 0) {
        //     const isBestFacultyQuery = query.isBestFaculty;
        //     console.log('isBestFaculty Query:', isBestFacultyQuery);

        //     results = results.filter(teacher => teacher.isBestFaculty === isBestFacultyQuery);
        //     console.log('Best Faculty Results:', results);
        // }
        ...
        if (query.RangeOftutor && results.length > 0) {
            ...
            results = results.filter(teacher => teacher.RangeWhichWantToDoClasses >= rangeQuery.$lte);
