...
import TeacherCard from "./Tutors/TeacherCard";
import StudentCardForTeacher from "./StudentCardForTeacher";
import TeacherProfileModal from "./Tutors/TeacherProfilePage";
import TeacherDetailsModels from "./TeacherDetailsModels";
...
const SearchResults = () => {
  ...
  const [classIds, setClassIds] = useState([]); // Move outside useEffect
  const [ClassNames, setClassNames] = useState([]);
  const [DisOpen, setDIsOpen] = useState(false)
  const [DselectedTeacher, SetDSelectedTeacher] = useState(null)
  ...
  const SearchPlaceLng = params.get("SearchPlaceLng");
      ...
      const fetchResults = async () => {
        try {
          const response = await axios.get(
            `http://localhost:7000/api/v1/teacher/Get-Min-search/${classIdParam}/${subjectParam}?lat=${latParam}&lng=${longParam}&ClassNameValue=${ClassNameValue}&role=${role}&SearchPlaceLat=${SearchPlaceLat}&SearchPlaceLng=${SearchPlaceLng}`
          );
          ;
  ...
  const handleDOpen = (teacherItem) => {
    if (teacherItem) {

      setDIsOpen(true)
      SetDSelectedTeacher(teacherItem)
    } else {
      setDIsOpen(false)
      SetDSelectedTeacher(null)
    }
  }
  const handleDClose = () => {
  ...
  const FetchTeacherClassNames = async (classIds) => {
    try {
      ...
      for (let index = 0; index < classIds.length; index++) {
        ...
        const classIdStr = element.ClassId.toString(); // Convert to string if necessary

        // Find the main class by its _id
        const foundClass = classData.find((cls) => cls._id.toString() === classIdStr);

        if (foundClass) {
          matchedClassNames[classIdStr] = foundClass.Class;
        } else {
          // If no main class match, search in InnerClasses
          let innerClassMatch = null;

          for (let cls of classData) {
            innerClassMatch = cls.InnerClasses.find((inner) => inner._id.toString() === classIdStr);
            if (innerClassMatch) {
              matchedClassNames[classIdStr] = innerClassMatch.InnerClass;
              break;
            }
          }

          if (!innerClassMatch) {
            console.log("No match found for ClassId:", classIdStr);
          }
        }
      }

      console.log("Matched Class Names:", matchedClassNames);
      setClassNames(matchedClassNames); // Update state with matched class names
    } catch (error) {
  ...
  return (
    <div className="search-results container py-4">
      ...
      <p className="lead">Total Found Results: {count}</p>
      <hr />
      ...
      <div className="results-container">
        <div className="row g-4">
          ...
          {role === 'student' ? (
                ...
                results.map((item, index) => {
                  ...
                  return (
                    <div key={index} className="col-lg-3 position-relative col-md-4 hs col-12">
                      ...
                      <div className='py-2 px-1  hs position-relative glass'>
                              ...
                              <div className="col-sm-12 mb-12">
                                <div className="contact-box d-flex justify-content-start gap-2 align-items-center">
                                  <div className="contact-icon d-flex justify-content-center align-items-center">
                                    <img src="https://i.ibb.co/G26rfx7/online-learning-1.png" width={20} alt="Online Learning Icon" />
                                  </div>
                                  <div className="contact-desc">
                                    {/* Move the JavaScript logic outside of the JSX */}
                                    {item.AcademicInformation.map(info => ClassNames[info.ClassId] || "Unknown Class").join(", ")}
                        ...
                        <div className='col-12 d-flex gap-2 align-item-center w-100'>
                          <button style={{ whiteSpace: 'nowrap' }} onClick={() => handleDOpen(item)} class="btn-shine w-75 buttonsss ">
                            <span>View More Details</span>
                          </button>
                          <button style={{ whiteSpace: 'nowrap' }} onClick={() => handleContactClick(item)} class="btn-shine w-75 buttonsss ">
            ...
            <StudentCardForTeacher result={results} />
          )}
      ...
      <TeacherProfileModal isOpen={DisOpen} isClose={handleDClose} item={DselectedTeacher} />
